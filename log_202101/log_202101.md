...
- 上記とは別に`config`フォルダに下記のファイルを格納しておく。
...
- `sbin/start-dfs.sh`の実行時、Dockerfile内でsshdを起動するコマンドを発行しているはずなのだが、なぜが実行されていない？ようで`3e6873e8fda4: ssh: connect to host 3e6873e8fda4 port 22: Connection refused`と「SSHが立ち上がってないぞ」と怒られる。理由は不明。
