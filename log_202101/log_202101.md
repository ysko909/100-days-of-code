...
HadoopをDockerコンテナ上に展開してみる。手順は[ここ](https://hadoop.apache.org/docs/stable/hadoop-project-dist/hadoop-common/SingleCluster.html)を参照。
...
RUN wget -q -O - https://ftp.tsukuba.wide.ad.jp/software/apache/hadoop/common/hadoop-3.2.2/hadoop-3.2.2.tar.gz | tar zxf -

ENV PATH=/hadoop-3.2.2/bin:$PATH
...
COPY config /hadoop-3.2.2/etc/hadoop/

RUN echo "export JAVA_HOME=/usr/local/openjdk-8" >> /hadoop-3.2.2/etc/hadoop/hadoop-env.sh

CMD ["/usr/sbin/sshd"]
```

- 上記が作成したDockerfileの中身。
...
- 上記を`core-site.xml`という名前で`config`フォルダに保存する。
...
- 上記を`hdfs-site.xml`という名前で`config`フォルダに保存する。

- これらのファイルはDockerfileで`COPY`のために利用される。
- vscodeから「Reopen in container」を実行する。
- 実行が終わったところで、下記のコマンドを実行する。
...
- 上記がすべて実行できたら[ここ](http://localhost:9870/)にアクセスする。ページが見られればとりあえず動作はOK。
...
- 最後の`cat`が出力されれば、疑似分散モードでの動作確認ができた。
- `<username>`は現状でログインしているユーザー。
- 最後の`cat`は`bin/hdfs dfs -cat output/*`でも同じ。

**思ったこと**:

- `sbin/start-dfs.sh`の実行時、Dockerfile内でsshdを起動するコマンドを発行しているはずなのだが、なぜが実行されていない？ようで`3e6873e8fda4: ssh: connect to host 3e6873e8fda4 port 22: Connection refused`と「SSHが立ち上がってないぞ」と怒られる。~~理由は不明~~多分だけど、実行して直後に終了してる感じ。このコマンドを実行した直後に中間コンテナが破棄されてたため、実行自体はできてもそのプロセスが残ってないって言った方が正しいか。永続化できるのかな、これ。
...
1. [HadoopをDockerコンテナで動かす](https://naoty.dev/posts/103.html)
1. [Error: JAVA_HOME is not set and could not be found. even though JAVA_HOME is set.](https://community.cloudera.com/t5/Support-Questions/Error-JAVA-HOME-is-not-set-and-could-not-be-found-even/td-p/99061)
1. [【Docker】社内プロキシ環境下でDocker Hubからイメージをpullしようとすると失敗する](https://www.godaiblog.com/entry/2018/12/17/%E3%80%90Docker%E3%80%91%E7%A4%BE%E5%86%85%E3%83%97%E3%83%AD%E3%82%AD%E3%82%B7%E7%92%B0%E5%A2%83%E4%B8%8B%E3%81%A7Docker_Hub%E3%81%8B%E3%82%89%E3%82%A4%E3%83%A1%E3%83%BC%E3%82%B8%E3%82%92pull%E3%81%97)
1. [分散処理に入門してみた（Hadoop+Spark）](https://www.casleyconsulting.co.jp/blog/engineer/150/)
...
Phaserでシーンの切り替えをする方法について。

- 切り替え元のシーンで`this.scene.start()`を実行する。
...
- 上記のような構成をしているという前提。`assets`フォルダにはなにもない。

```typescript
import * as React from 'react';
import * as ReactDOM from 'react-dom';
import * as Phaser from 'phaser';
import * as GameConfig from './ts/gameConfig'
import { Scenes } from './scene/sceneIndex'
...
const config: Phaser.Types.Core.GameConfig = {
  type: Phaser.AUTO,
  parent: 'root',
  width: GameConfig.gameValues.width,
  height: GameConfig.gameValues.height,
  backgroundColor: '#e0e0e0',
  scale: {
    mode: Phaser.Scale.FIT,
...
- `index.tsx`の内容は上記のとおり。ゲームの実行設定と実行しかしていない。
...
- `sceneIndex.ts`の内容は上記のとおり。シーンをロードしている。
...
export class LoadingScene extends Phaser.Scene {
    ...
    create() {
        ...
        let r1 = this.add.rectangle(150, 150, 100, 150, 0x6666ff);
...
- `loadingScene.ts`は上記のとおり。クリックでシーンが遷移するよう、`zone`を設定している。`useHandCursor`プロパティをtrueにすると、カーソルが手の形になる。
...
export class TitleScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(width / 2, height / 2 + 60, 'Title Scene').setOrigin(0.5);
...
- `titleScene.ts`の内容は上記のとおり。基本的に`loadingScene.ts`と変わらない。文言が違うだけ。
...
export class MainScene extends Phaser.Scene {
    ...
    create() {
        ...
        let r1 = this.add.rectangle(250, 250, 100, 150, 0x6666ff);
        ...
        this.add.text(width / 2, height / 2 + 60, 'Main Scene').setOrigin(0.5);
...
- 最後の遷移先である`mainScene.ts`。ここより先には遷移しないので、`this.scene.start()`は記述していない。

**思ったこと**:

- 一方通行にするなら、この切替方法が一番カンタン。

**リンク**:

1. [Phaser.Scenes.SceneMagager](https://photonstorm.github.io/phaser3-docs/Phaser.Scenes.SceneManager.html#start__anchor)
1. [Phaser 3 | シーンを切り替える方法](https://1-notes.com/chenge-scenes/)

### 119日目: 2021年1月29日（金）

**今日の進捗**:

Gitでリモート先を変更する方法について。

- `git remote set-url`を使う。
...
- 上記のコマンドを実行する前に、`git remote -v`で現状のURLを確認しておこう。

**リンク**:

1. [gitのremote urlを変更する(レポジトリ移行時)](https://qiita.com/minoringo/items/917e325892733e0d606e)

### 120日目: 2021年1月30日（土）

**今日の進捗**:

ReactアプリでPhaser3を使う場合の、アセット保存場所について。

- `create-react-app`で作成したアプリケーションにPhaserをインポートしてゲームを作成する場合、アセットは`public`フォルダ内に作成しないといけない。
...
- 上記のように、`public`フォルダに`assets`フォルダを作成して、その中に任意のファイル名で保存する。この際、フォルダに格納しても問題ない。あくまでも、`public`フォルダ内にありさえすればいい。
- 逆に`src`ファイル内にアセットを保存してしまうと、アプリ実行時にアセットが見つからないため、本来アセットが表示されるべきところに何も表示されないことになってしまう。
- これはReact側の仕様。`public`フォルダにアセットを保存しておくと、`public`フォルダをベースURLを解釈するので、プログラム側で「どこのフォルダにアセットが保存されているか」を考慮せずに済む。
...
export class MainScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('piyo', 'assets/if/piyo.png');
    }
    ...
    create() {
        ...
        this.add.image(width - 100, height - 100, 'fuga');
        this.add.image(width - 200, height - 200, 'piyo');
...
- `this.load.image()`の際、`assets/if/hoge.png`とファイルを指定している。これは`public/assets/if/hoge.png`でも同じ意味で、`public`フォルダの配下であればどちらの指定でも使える。
- つまり、`public`フォルダに格納されていれば、そこをルートとして考えられるのでファイル管理や指定が楽になる、ということ。

**思ったこと**:

- ReactAppでPhaser使ってるやつなんてあんまりいないと思ってたから、ググって答えが出てきたときは少し感動すらしてしまった。

**リンク**:

1. [This.load.image file path not working](https://phaser.discourse.group/t/this-load-image-file-path-not-working/1566)
1. [Using the Public Folder](https://create-react-app.dev/docs/using-the-public-folder/)

### 121日目: 2021年1月31日（日）

**今日の進捗**:

Phaser3で画像のスケールを変更する方法について。

- `scale`を使うと、画像のサイズを拡大あるいは縮小できる。
...
- `scale`だと縦横どちらも拡大あるいは縮小する。`scaleX`ならX軸、つまり横方向に拡大あるいは縮小する。`scaleY`ならY軸、つまり縦方向に拡大あるいは縮小する。

**思ったこと**:

- 拡大・縮小は`scale`。覚えた。
- 100日を超えたことと月末日でキリがいいので100daysofcode終了！これからはTIL（Today I Learn）として、別リポジトリで記録していこう。

**リンク**:

1. [Phaser.GameObjects.Image](https://photonstorm.github.io/phaser3-docs/Phaser.GameObjects.Image.html)
1. [Scaling in phaser3](https://phasergames.com/scaling-in-phaser-3/)
