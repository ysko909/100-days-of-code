...
1. [最小二乗法でカーブフィッティング。関数３つを使い比べ－python](http://ailaby.com/least_square/)
...
- 「部分従属プロット」とは、学習モデルの解釈性を向上するための一手法。その学習モデルが予測する際、どの説明変数に対して**どのように影響を与えているか**を見るもの。
- たとえばRandomForestではFeature Importanceで、どの説明変数が学習モデルに影響を与えたかまではわかる。ところが「どのように」はわからない。ある特徴量の値が小さいと予測値が大きくなるとか、あるいはその逆といった影響を見たい場合には部分従属プロットを用いる。
- ここまでで、「じゃあSHAP」とはどう違うの？という話なのだが、PDPは「特徴量がどのように影響を与えているか」であり、SHAPは「予測結果に対して特徴量がどう寄与しているか」の違いがある。SHAPは「この予測結果だったらどの説明変数がこれこれこんな具合に寄与している」と表現するが、PDPは「（予測結果はともかく）説明変数の影響を表現する」ところが異なる。

----

交互作用についてメモ。

- 2つの因子が合わさることではじめて現れる相乗効果のこと。交互作用による効果を「交互作用効果」という。
...
- 基礎、ってほうはあとでnotebookでやってみる。

----

JavaScriptでオブジェクトの一覧を取得する方法について。

- JSでオブジェクトの内容を「とりあえず一覧化して見てみたい」っていうときに使う。
- Codepenなんかのプレイグラウンドだと、そこそこ大きいオブジェクトは内容を表示してくれないか省略しちゃって見たいものが見られなかったりするので。
...
console.log(Object.values(hoge));

for (key in hoge){
  console.log(hoge[key]);
  console.log(hoge.key); // これはダメ
}

for (let [key, value] of Object.entries(hoge)){
  console.log('key: ', key, '/ value: ', value);
}
```

- ループに使う変数（ここでは`key`）でオブジェクトのプロパティを指定する場合、ドット表記での記述方法はうまくいかない。
...
- 上記の通り、ドット表記したものは`undefined`になってしまう。これは`hoge.key`というプロパティを参照しようとして「そんな名前のプロパティは`hoge`オブジェクトにないよ」と言われているため。
...
- ここで`key`という名前のプロパティを追加して、同じfor文を実行してみる。
...
- プロパティ名が`key`である値を表示している。このことから、ドット表記した`key`は変数ではなくプロパティ名として解釈されていることがわかる。
- 本来はすべてのキーを順番にアクセスしてもらいたい（実際、変数`key`にはプロパティ名が順番に入っている）のに、ドット表記は「記述されたコードを変数名だと解釈せずに**プロパティ名だと解釈する**」せいで、`[]`で囲う[ブラケット表記](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Operators/Property_Accessors)したものと結果が異なってしまう。
- 可読性の面からドット表記を多用しがちだし簡単なのだけど、上記のような罠があることは覚えておきたい。
- この場合はfor文で回すより`Object.entries()`なんかのメソッドを使った方がいいかもしれない。

**思ったこと**: 

- 正直、どっちもまだ咀嚼しきれてないんですけどね・・・。
- 普段は似た挙動をするのに、あるときだけ異なる挙動をするのってすごい困るんですけどね・・・。

**リンク**

1. [SHAP値で解釈する前にPermutation ImportanceとPDPを知る](https://research.miidas.jp/2019/11/shap%E5%80%A4%E3%81%A7%E8%A7%A3%E9%87%88%E3%81%99%E3%82%8B%E5%89%8D%E3%81%ABpermutation-importance%E3%82%92%E7%9F%A5%E3%82%8B/)
1. [4.1. Partial dependence plots](https://scikit-learn.org/stable/modules/partial_dependence.html)
1. [edarf パッケージ：ランダムフォレストで探索的データ分析](https://qiita.com/nakamichi/items/bed7a2f180ea9ce86d94)
1. [Python: 学習済み機械学習モデルの特性を PDP で把握する](https://blog.amedama.jp/entry/pdp)
1. [交互作用・多項式(特徴量エンジニアリング) ](https://note.com/okonomiyaki011/n/nf0fb8d8b93f6)
1. [交互作用とは](https://bellcurve.jp/statistics/blog/14335.html)
1. [Pythonによる時系列分析の基礎](https://logics-of-blue.com/python-time-series-analysis/)
1. [時系列データに対する特徴量エンジニアリング手法のまとめ](https://yolo-kiyoshi.com/2019/03/23/post-1111/)
1. [実務の現場に多い時系列データ分析の際に注意しておきたい点を列挙してみる](https://tjo.hatenablog.com/entry/2017/09/22/190000)
1. [プロパティアクセサー](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Operators/Property_Accessors)
1. [JavaScriptにおける連想配列のforループ操作](https://qiita.com/wifeofvillon/items/15359535a834832e08ea)
1. [Object.values](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Object/values)
1. [Object.keys](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Object/keys)
1. [Object.entries](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Object/entries)

### 29日目: 2020年10月30日（金）

**今日の進捗**: 

JavaScriptでファイルをアップロードして情報を参照する方法について。

- ファイルのアップロードにはJSのFile APIを利用する。
...
- ファイルを選択するには`input`タグを利用する。ここで`type`は`"file"`を指定する必要がある。
...
- changeイベントが発生するタイミングは「ファイルが選択された」とき。
- 読み込んだファイルの情報は`e.target`で参照できる。この中では`Filelist`と`Files`の2つのオブジェクトがある。`FileList`では選択したファイルの数がわかるlengthプロパティがあり、`File`オブジェクトには、nameプロパティなどの個別のファイルに関する情報について参照できる。

```console
<input id="file1" type="file" name="file1">
FileList [ File ]
1
File { name: "work.html", lastModified: 1603954061123, webkitRelativePath: "", size: 724134, type: "text/html" }
```

- `e.target.files`とするとFilelistオブジェクトを指し、`e.target.files[0]`とすると個別のFileオブジェクトを指す。さらに`e.target.files[0].name`などとプロパティ名を指定し参照することでFileオブジェクトの情報についてアクセスできる。

----

アップロードしたファイルの中身を参照する方法について。

- アップロードしたファイルにアクセスする場合は、FileReaderオブジェクトを利用する。
...
- FileReaderオブジェクトをインスタンス化し、アップロードしたファイルを`readAsText()`に渡す。今回はHTMLファイルをアップロードすることを想定しているので、ファイルをテキストとして扱うために`readAsText()`を使う。

----

JavaScriptのスプレッド構文について。

- JSで`...`とドットが3つ並んでるのをみたら、それは「スプレッド構文」という書き方。

```javascript
const hoge = [1, 2, 3];
const fuga = ['foo', 'bar', 'baz'];
...
- 配列の前に`...`とすると、要素を個別に取り出す。
- `...hoge`と書くと、配列`hoge`内の要素をすべて取り出し、個別の値として扱う。よって、`1, 2, 3`と記述することと同値になる。
...
- 文字列でも同様。
- 上記のような、「配列中の要素を個別の値に展開する」場合によく用いる。
...
console.log(Math.max(hoge)); // この書き方では最大値を取得できない
console.log(Math.max.apply(null, hoge)); // このように書く必要はない
```

- たとえば配列中の最大値を取得したい場合は`Math.max()`を用いるが、引数にスプレッド構文で要素を個別の値に展開することで、簡単な書き方をしつつ実装も楽にしてくれる。
...
- 配列同士の連結も、スプレッド構文を利用することで簡単に記述できる。

```javascript
let num = 10;

[...Array(num).keys()].forEach(j => {
  console.log(j);
});
...
- for文を使わずforEach文でループを回す方法もある。まぁ、これについては「こんな書き方もある」っていう程度でいいかなっていう気もする。

**思ったこと**: 

- ファイルをアップロードするだけなら、ちょっとだけJS書けば実装できるんだ、便利だなー。
- スプレッド構文、いろいろ便利に使えそうなので覚えておこう。

**リンク**

1. [XMLHttpRequest でファイルをアップロードする方法](https://javascript.keicode.com/newjs/upload-files-with-xmlhttprequest.php)
1. [JavaScriptでFile APIを用いたファイル操作方法ついて解説](https://www.e-loop.jp/knowledges/35/)
1. [【JavaScript】スプレッド構文の便利な使い方まとめ](https://qiita.com/Nossa/items/e6f503cbb95c8e6967f8)

### 30日目: 2020年10月31日（土）

**今日の進捗**: 

p5.jsのパーリンノイズを使った曲線の描画についてメモ。

- p5.jsのパーリンノイズを使って、時間で緩やかに変化する曲線を描画する。
...
function draw(){
  ...
  [...Array(lineNum).keys()].forEach((j) => {
    ...
    const hue = Math.round(j / lineNum * 60) + 200;
    const saturation = 100;
    const brightness = Math.round(j / lineNum * 75) + 25;
...
JavaScriptでのドル記号について。

- JavaScriptでは、ドル記号は特殊な意味を**持たないただの文字**として**基本的には**扱われる。
- ただ、[テンプレートリテラル](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Template_literals)のプレースホルダとして利用したとき、ドル記号は「ここはプレースホルダですよー」と表現するための特殊な意味を持つようになる。
...
let piyo = `ham eggs spam ${fuga}`;

$(piyo);
```

- 上記の通り、ドル記号1つだけという名前の関数を作ることが可能。そして当たり前だが普通に動作する。
- ただし、バッククォートで囲ったテンプレートリテラルの場合、文字列中に`${}`が存在する場合はその部分をプレースホルダとして扱うようになる。

```console
hogehoge
ham eggs spam foo bar baz
```

- プレースホルダに変数を指定することで、その変数が持っている値を出力することが可能。

```javascript
let hoge = 1010
let fuga = `My number is ${hoge + 3 * 123}`;
let piyo = `ham eggs spam ${'foo' + 'bar' + fuga}`;

$(piyo);
```

- プレースホルダ内では計算式を記述することもできる。そして当たり前のように数値を格納している変数と計算することが可能。

```console
ham eggs spam foobarMy number is 1379
```

- 注意点としては、ぱっと見で分かりにくいものの**バッククォートで囲われた中でしか動作しない**ということ。
...
- シングルクォーテーションで囲われている場合、それはただの文字列なのでドル記号と波かっこを記述しても文字列として扱われる。

```console
ham eggs spam ${fuga}
```

- IDEによるのかもしれないが、シングルクォーテーションで囲った場合とバッククォートで囲った場合で**同じ色を使って表示しているケースがある**。こういうときはテンプレートリテラルなのかただ文字列なのか、ぱっと見で判断しにくい。
- テンプレートリテラルを多用するような場合は、文字列はシングルクォーテーションを利用せず、ダブルクォーテーションを利用するようにルール化した方がいいかもしれない。
...
- ちなみにテンプレートリテラルを使いつつ、`${}`部分を文字列として出力したい場合は、バックスラッシュをつけてエスケープする。

```console
ham eggs spam ${fuga}
```

- まぁ、こういうケースならそれこそシングルクォーテーションかダブルクォーテーションを利用しようね、という話。

**思ったこと**: 

- こういうコーディングがパッと書けるようになるといいんだがなぁ。まずは三角関係の勉強し直しからかな。
- テンプレートリテラル、使いこなせると便利そうだけどぱっと見で判断できないとバグが潜みそうで厄介かもしれないな。

**リンク**

1. [JavaScriptで取り組むクリエイティブコーディング パーリンノイズを使いこなせ](https://ics.media/entry/18812/)
1. [テンプレートリテラル](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Template_literals)
1. [JavaScript の テンプレートリテラル を極める！](https://qiita.com/kura07/items/c9fa858870ad56dfec12)
...
- 画面に動きを出すため、`(0,0)`固定だった点の座標を時間経過とともに変更する。
- 座標を変更するためには三角関数を利用して角度（ここでは`theta`）とラジアン（ここでは`r`）の2つを利用する。[弧度法](https://ja.wikipedia.org/wiki/%E3%83%A9%E3%82%B8%E3%82%A2%E3%83%B3)というやつ。

> 180度 = PI * rad

- 「xx度」で角度を表現する方法は[度数法](https://ja.wikipedia.org/wiki/%E5%BA%A6_(%E8%A7%92%E5%BA%A6))と呼ばれる。
- 度数とラジアンの表現は上記の`PI * rad`が180度であることを基準にして、これの2倍なら360度、1/2なら90度・・・という風に計算できる。
