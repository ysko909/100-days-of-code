...
- なお、`--template typescript`とせず`--typescript`と記述しているページ（リンク参照）を見つけた。どうもエイリアスになっているようで、試しに`--typescript`で実行してみたがエラーにはならなかった。ただ、オフィシャルのドキュメントに記述してある方法は`--template typescript`なのでこちらを採用することとする。
