...
- JS側で**リンク**:を生成する。この際、作成した要素に対し値を設定するのが`setAttribute()`。
...
- ダウンロード**リンク**:を作ることも考えたが毎回クリックして保存先選んでDLしてDL先のフォルダを開いてファイル選択してパワポに貼り付けて・・・ってやりたくなかった。
...
- Unityエディタ上のProjectタブを選択して、ProjectウィンドウのAssets内の適当なところで右クリックメニューを表示し、「Create」から「C# Script」を選択する。
...
1. [MonoBehaviour](https://docs.unity3d.com/ja/current/ScriptReference/MonoBehaviour.html)
1. [Unity2019 + mac + VisualStudioCodeで、補完が効かなくなった時の対処メモ](https://qiita.com/matsuyoro/items/060c761104cffd5b6346)
...
- 注意点は、`char`型に値を格納する場合はシングルクォーテーションを用いるのに対し、`string`型に値を格納する場合はダブルクオーテーションを用いる。文字列はダブルクオーテーションなので、デバッグの際に出力する任意の文字列なんかもダブルクオーテーションで囲う。
...
− 小数を扱う際、 `0.1f`のように末尾に`f`をつけるとfloat型として扱う。これがない場合はdouble型として扱う。`float hogera = 0.1;`のように、floatで宣言した変数に`f`なし（double型）に代入はできない。この場合、変数の型と代入する値の型が一致しないよ」とエラーになる。

> Literal of type double cannot be implicitly converted to type 'float'; use an 'F' suffix to create a literal of this type [Assembly-CSharp]

- この逆、つまり`double hogera = 0.1f;`のように、double型に対し`f`を付与したfloat型を代入するのは可能。

----

四則演算のやり方について。

- なんとなく予想はつくと思うけど、四則演算の記述方法は下記のとおり。
...
- 多分、大抵のプログラミング言語で一般的な記述方法と同じだと思う。
...
- インクリメントやデクリメント、任意の四則演算をしながら変数に代入する簡易表記も可能。
...
- PythonやJavaScriptはクォーテーションはシングルでもダブルでも文字列扱いになるから、そこが異なるところ。

**リンク**:

1. [文字と文字列](https://programming.pc-note.net/csharp/character.html)
...
- これもなんとなく予想がつくと思うけど、`+`で文字列同士を連結できる。簡易表記も使える。
- 文字列と数値を連結できる。この場合、数値は文字列として扱われる。
...
C#での配列の利用方法。まずは基本的なところ。

- 配列の宣言自体は`int[] hoge;`などで可能。ただし、**宣言しただけでは利用できない**。じゃあどうすればいいかというと、`new`というキーワードを使ってインスタンスを作成する。つまり`int[] hoge = new int[3];`と記述する。これで利用可能な準備が整う。
- 配列の宣言はあくまで「どんな型の配列か」を宣言するだけで、実態は配列型のクラスでインスタンスを要素数を指定しつつ作成するため`new`が必要という考え方らしい。

**思ったこと**:

- このあたりはプログラミング言語特有のものって少ないので覚えやすい。
...
HTMLに含まれるSVGファイルをPNGにして表示するReactのツールの続き。

- またコードを清書してないが、`Promise`を利用することでPNG出力ができるようになった。
...
function generateDownloadURLs(svgText, key, callback) {
...
function RenderSVG(props) {
    ...
    useEffect(() => {
        if (Array.isArray(props.value) && Object.keys(props.value).length > 0) {
            ...
            const elem = document.getElementById('imgs');
            const elemUrls = document.getElementById('urls');
            ...
            props.value.forEach((svgText, idx) => {
                ...
                loadImage(svgText).then(res => {
                    console.log('back url:' + res);
                    result.push(res);

                    elem.insertAdjacentHTML('beforeend', '<img key=img_' + idx + ' alt="" src=' + ('data:image/svg+xml;charset=utf-8;base64,' + btoa(unescape(encodeURIComponent(svgText)))) + '></img>');

                    elemUrls.insertAdjacentHTML('beforeend', '<img key=img_' + idx + ' alt="" src=' + res + '></img>');
                }).catch(e => {
                    console.log('error: ' + e);
                });
            });

            elem.insertAdjacentHTML('beforebegin', '<h3>SVGs</h3>');
            elemUrls.insertAdjacentHTML('beforebegin', '<h3>PNGs</h3>');
...
JSの`import`と`export`について。

- `export`の設定時に**デフォルトエクスポートを設定できる**。デフォルトエクスポートが設定されている場合、`import`した側で任意の名前を利用できる。
- デフォルトエクスポートは1つしか設定できない。
...
- `import`したJSファイルでは`hoge`という名前で呼び出さなくてもよい。これはデフォルトエクスポートが設定されているため、「とりあえずインポートするのは`hoge`なんでしょ？なら別に`hoge`の名前をそのまま使わなくてもいいよね」ということで、別名をつけることが可能になっている。
- デフォルトエクスポートが設定されていない、名前付きエクスポートの場合は別名によるインポートは不可能なので注意が必要。

----

curlのこと。

- Dockerのコンテナにbuster-slimを使っていたら`curl`がいなかった。ので、下記のコマンドを実行してインストールした。
...
1. [Promise](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Promise)
1. [Export](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Statements/export)
1. [Can't run Curl command inside my Docker Container](https://stackoverflow.com/questions/34571711/cant-run-curl-command-inside-my-docker-container)
...
public class work_enemy : MonoBehaviour
{
    ...
    void Start()
    {
        int[] hoge = { 10, 20, 30, 40, 50 };

        for (int i = 0; i < 5; i++)
        {
            Debug.Log("now: " + hoge[i]);
        }
    }
...
- 配列宣言しつつ波括弧で要素を代入する。この場合、代入する要素数は確定しているので`new`で配列の要素数を指定する必要はない。
...
- なお、ある配列の要素数を取得したい場合は`配列名.Length`で取得できる。
...
public class work_enemy : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach (int number in hoge)
        {
            Debug.Log("now: " + (number + 9));
        }
    }
...
- 2次元以上の多次元配列の場合、`foreach`よりは入れ子の`for`で記述した方が[厳密に制御できる](https://docs.microsoft.com/ja-jp/dotnet/csharp/programming-guide/arrays/using-foreach-with-arrays)、とのこと。

----

メソッドを書く基本。

− メソッド宣言の形式はCでの関数宣言と似ていて、戻り値の型を宣言しメソッド名、引数の有無を記述する。また、**戻り値は1つしか指定できない**。このあたりはPythonなどとは異なる。
- メソッドを記述する順番は、必ずしも呼び出す前に限定されないらしい。
- なお、メソッド名は大文字で始まるキャメルケースっぽい。
...
- メソッドのブロック中で変数を宣言して、その変数に計算結果を代入し`return`する方法もあるが、上記のように直接`return`に計算式を記述しても結果は同じ。
...
- 戻り値を`void`に設定すると、戻り値がないメソッドである宣言になる。
...
public class work_enemy : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < hoge.Length; i++)
        {
            DisplayNumber(hoge[i]);
        }
    }

    void DisplayNumber(int num)
    {
        Debug.Log("from method: " + num);
    }
...
1. [配列での foreach の使用 (C# プログラミング ガイド)](https://docs.microsoft.com/ja-jp/dotnet/csharp/programming-guide/arrays/using-foreach-with-arrays)
...
- 注意点は、インスタンス生成時に`new`をつけることと**クラス名にカッコをつけること**。今回は利用していないがコンストラクタを使ってクラスの既定値を変更する場合は、このカッコの中に値を記述する。
...
- `public`は`private`はアクセス修飾子という。
- 他のクラスからメンバに対しアクセスを許可するか、アクセス修飾子を使って制御する。`private`はそのクラスのみが利用できるが、`public`は他のクラスからもアクセス可能。
...
- 上記の場合、クラス`Player`そのものは他からアクセス可能。メソッド`Attack()`や`Damage()`もアクセス可能だが、変数`hp`や`power`はアクセスができない。
- アクセス修飾子を省略した場合は`private`として扱われる。
- `protected`というアクセス修飾子もあり、これは**同じクラスとそのサブクラスからのみアクセス可能**である。

----

C#の`this`について。

- `this`は**自分自身のインスタンスを指すキーワード**。感覚してはJavaScriptのそれと似てる。
...
- メンバ変数を利用する場合は**明示的に**`this`を付与することで、バグの発生を防ぐことができる。

----

クラス利用の練習に`Vector2`クラスを使ってみる。

- `Vector2`クラス（なんか正確には構造体らしいが）は、2Dゲーム用のベクトルを表す。2D用なので、x軸とy軸の2つ。`Vector3`クラスになると、これにZ軸が追加されて3軸になる。
...
public class work_enemy : MonoBehaviour
{
    ...
    void Start()
    {
        Vector2 playerPosition = new Vector2(0.5f, 1.5f);
        playerPosition.x += 5.5f;
        playerPosition.y += 4.2f;
...
- `Vector2`クラスはfloat型の値を持つため、コンストラクタに既定値を記述する際は`f`を付与する。

```console
(6.0, 5.7)
UnityEngine.Debug:Log (object)
work_enemy:Start () (at Assets/work_enemy.cs:13)

```

- `Debug.Log()`には宣言したクラスの名前しか記述していないが、xとyがそれぞれ表示されている。とりあえず、こういうものらしい。ちなみに、自分で作ったクラスで同じことをやってみたがこういう表示にならなかったので、構造体は条件が違うかあるいは優先して戻る値を設定しているかのどちらかだろうと思われる。

- 次は`Vector2`クラス同士で計算をしてみる。
...
public class work_enemy : MonoBehaviour
{
    ...
    {
        Vector2 playerPosition = new Vector2(0.5f, 1.5f);
        Vector2 enemyPosition = new Vector2(3.0f, 2.5f);
...
- 宣言した`Vector2`クラスの変数同士で計算（ここでは減算）してみる。
...
- `x`と`y`それぞれが計算されたことがわかる。

**思ったこと**:

- 変数名を`Debug.Log()`に渡しただけでメンバ変数の値が表示されるのはいまいち咀嚼しきれてないが、そういうものだとここでは覚えることにする。

**リンク**:

1. [クラス (C# プログラミング ガイド)](https://docs.microsoft.com/ja-jp/dotnet/csharp/programming-guide/classes-and-structs/classes)

### 84日目: 2020年12月24日（木）

**今日の進捗**:

Unityでサンプルのゲームプロジェクトを開発する。下準備編。

- プロジェクトを作成する。作成手順はいつもと同様で、今回は2Dゲームを作成する。プロジェクト名は任意。
- プロジェクトに素材を追加する。素材を追加する場合は、素材をProjectタブのAssets内にドラッグアンドドロップする。
- スマートフォンで動作するアプリを作るための設定を行う。
  - Fileメニュー内のBuild Settingsを選択する。
  - Build Settingウィンドウが開いたら、Platform欄からiOSあるいはAndroidを選択して、Switch Platformボタンをクリックする。
  - 数秒かかって設定が変わる。設定変更が終了したらBuild Settingウィンドウは閉じてしまっても構わない。
  - ビルド先を変更することで、それぞれのスマートフォン向けにプロジェクトをビルド可能になる。
- 画面サイズの設定を行う。
  - Gameタブをクリックして、画面サイズ設定（Aspect）のドロップダウンリストから、対象にしたいスマートフォンの画面サイズにあったものを選択する。
  - ここでは「iPhone X/XS 2436x1125 Landscape」を選択する。
  - Aspectの選択が終わったらSceneをクリックしてシーン画面に戻る。
- シーンを作成・保存する。
  - FileメニューからSave Asを選択する。Save As欄に保存したいシーンの名前を記述し、Saveボタンを押す。
  - 一度シーンを作成・保存すると、以降はFileメニュー内のSaveを押すだけで上書き保存される。
  - ここまでがゲームを作成する下準備になる。
  
----

Unityでサンプルのゲームプロジェクトを開発する。オブジェクト追加と操作編。

- シーンの作成までが終了したら、シーンにオブジェクトを追加する。
  - シーンに追加できるオブジェクトは、事前にAssetsに追加されたオブジェクトに限定される。追加できていない場合は、事前にAssetsに追加しておく。追加方法は、FinderなどからドラッグアンドドロップでUnityのAssetsに追加してやればいい。
  - 追加したいオブジェクトをAssets内で選択し、Sceneにドラッグアンドドロップすれば追加される。
  − 位置合わせは操作ツールで移動させるか、InspectorのPositionを編集することで変更できる。大まかな移動は操作ツールで変更した方が早いが、特定の座標を設定したい場合はInspectorを利用するほうが早い。
  - X座標およびY座標をそれぞれ0にすると、シーン（ゲーム画面）の中央に配列できる。
  - なお、2Dゲームを選択した場合でもZ軸は存在する。この場合、基本的にオブジェクトのZ軸は**0を設定しておくこと**。これは、シーンを映すカメラがZ軸の-10に配置されているためで、オブジェクトのZ軸の値が-10より小さいとカメラより手前に位置してしまうため、ゲーム画面内に映らなくなってしまう。
- 背景色を変えることができる。
  - ヒエラルキーウィンドウでMain Cameraを選択する。
  - Inspector内のCamera項目にBackgroundというメニューが存在するので、カラーバーを選択しColorウィンドウから任意の色を選択できる。ここでは#242A2Cを選択した。
  - なお、色の指定は16進数形式で指定することも可能。指定したい色のカラーコードがわかっている場合は、そっちを指定した方が早い。

----

Unityでサンプルのゲームプロジェクトを開発する。スクリプトの作成編。

- オブジェクトを配置しただけではゲームとして成立せず、何かしらの機能を実装しないといけない。この「機能」を実装するために必要なものがスクリプト。この前までC#で書いてたアレのこと。
- ここでは「クリックされるとオブジェクトが回転する」スクリプトを実装する。
- スクリプトはプロジェクトウィンドウ内で右クリックし、表示された右クリックメニューからCreate内のC# Scriptを選択する。ファイル名は任意。ファイルが作成されたらダブルクリックしてエディタを開く。
...
public class script_for_motherboard : MonoBehaviour
{
    // Start is called before the first frame update
...
- スクリプトには上記の内容を記述し保存する。
- スクリプトを書いただけでは動作せず、**どのスクリプトをどのオブジェクトにアタッチするか**を指定する必要がある。アタッチして指定さえすれば、スクリプト中でどのオブジェクトを指すのかわからなかった`this`も**アタッチした先のオブジェクトを指すようになる**。
- アタッチの方法は、Projectタブ内で表示されているスクリプトファイルを、ヒエラルキーウィンドウ内になるオブジェクトにドラッグアンドドロップする。この操作でスクリプトがオブジェクトにアタッチされる。
- アタッチが終了したら一度ゲームを実行してみる。ゲーム画面の任意の場所をクリックすると、オブジェクトが回転するはず。
- このままだとただ回転するだけなので、回転する分の値に減衰係数を掛ける。
...
- 減衰係数としてここでは0.98を掛けている。

----

Unityでサンプルのゲームプロジェクトを開発する。スマートフォンでの動作確認方法編。

- 作成したプロジェクトは先に指定したプラットフォーム先に対応したビルドが可能。実機で検証するには**実機と接続するためのUSBケーブルが必要**。
- iOSの実行ファイルには端末上でユニークな名前を付与しておく必要があるので、名称の設定を行う。
  - FileメニューからBuild Settingを選択する。Build Settingウィンドウ内のPlayer Settingボタンをクリックする。
  - 下の方にあるOther Settings内のBundle Identifierに`com.自分の名前などユニークになりそうな名称.アプリ名`を入力する。
  - この際、Bundle Identifierがグレーアウトして編集できない場合は、`Override default bundle identifier`にチェックを入れる。（今回はデフォルト設定でチェックがOFFになっていたのでチェックをONにした。[参考書](https://www.sbcr.jp/product/4815602543/)ではこのあたりに言及がなかったので、バージョンで違うのかもしれない。）
  - 入力が終わったらBuild Settingウィンドウに戻る。
  - Build Settingウィンドウの上部にあるScene in build欄で（おそらくデフォルトで）チェックがONになっているScenes/Sample Sceneのチェックを外す。
  - Unityのメイン画面のProjectタブからさっき保存したシーンを、Scene in build欄にドラッグアンドドロップする。チェックが入っていることを確認する。これで作成したシーンがゲームのビルド対象になる。
  - 設定ができたらBuildボタンを押す。
  - （多分）初めてのビルドでは、ビルド先を指定する。任意の場所を指定できるが、プロジェクト用のフォルダに`build`などの名前でフォルダを作成し、そこをビルド先にするのがわかりやすいと思う。
  - ビルドが完了すると、自動的にFinderがプロジェクト用フォルダを表示する。ここで、ビルド先のフォルダから`*.xcodeproj`という拡張子のファイルをダブルクリックする。Xcodeが起動されるはず。この段階でXcode側にインストールが必要なコンポーネントなどがある場合、一度インストールしてXcodeを終了してから再度`*.xcodeproj`ファイルをダブルクリックする。
  - とりあえずXcodeが起動したら、左のツリーのルートであるプロジェクトの名前をクリックして選択する（これをしないと次のステップに進めない）。
  - 次に、現れたメニューたちからSiging & Capabilitiesタブをクリックする。Xcodeのバージョンによっては、この項目がもともとGeneralタブにいたらしく、そっちをクリックしろと書いている参考書があるが、現状（2020年12月）時点ではGeneralタブでなくなっている。
  - Automatically manage signingがチェックされていることを確認したらAdd Accountボタンをクリックする。
  - AppleIDの入力を求められるので入力する。サインインが正常に完了したら、Accountsウィンドウの左側（AppleIDsと書いてある部分）にアカウントが表示されているはず。表示されているのが確認できたら、Accountsウィンドウは閉じてしまってよい。
  - Xcodeの画面に戻り、SigningのTeamが`None`になっているはずなので、ドロップダウンリストから自分のアカウントを選択する。なお、この時点でスマートフォンが端末に接続されてない場合、Statusとして赤丸に白くバツの書かれたアイコンが表示され、エラーメッセージが出力されているはず。iPhoneなどを端末に接続してTry Againボタンをクリックする。
  - ここまで完了したらXcodeでの設定は終了なので、画面上部の実行ボタン（再生ボタンみたいなやつ）をクリックする。多分、めちゃくちゃ多くのワーニング（黄色い三角に白い！マーク）が表示されると思うが、とりあえずビルドが終わればそれでいい。
  - 初回の実行で「Could not launch "アプリ名"」というエラーメッセージが出力された場合、iPhone側の設定が必要になる。iPhoneの設定＞一般＞プロファイルとデバイス管理画面において、デベロッパAPPとしてアプリを登録する必要がある（多分だけど初回だけ）。
  - また、iPhone側が画面ロックされているとアプリが転送されないので、**画面ロックは解除しておく**。
  - さらにiPhone画面を縦向きでロックしている場合、横向きのアプリは画面が表示されない（起動直後のUnityのスプラッシュウィンドウだけ表示される）。よって、**縦画面固定のロックも解除しておく**。
  − ここまでしておくと、Xcodeで実行すれば自動的にiPhone側に転送かつアプリが起動されるはず。

**思ったこと**:

- Xcodeのビルドはわりとおっかなびっくりだが、なんとか実機で確認できた。よかった。
...
前日までやってたUnityでのゲーム開発のまとめ。

- ゲームの設計は以下の手順で進める。
  1. 画面上のオブジェクトをすべて書き出す。
  1. オブジェクトを動かすためのスクリプトについて仕様を決める。
  1. オブジェクトを自動生成するためのスクリプトについて（必要に応じて）仕様を決める。
  1. UIを更新するスクリプトについて（必要に応じて）仕様を決める。
  1. スクリプトを作成する。
...
function draw(){
  ...
  [...Array(Math.ceil(width / size) + 1).keys()].forEach(w => {
    [...Array(Math.ceil(height / size) + 1).keys()].forEach(h => {
      if ((w % 2) === 0 && (h % 2) === 0){
        // X座標、Y座標、円の縦、円の横
        ellipse(w * size, h * size, size * extend, size * extend);
      }
...
- 水玉模様自体は、任意の背景色を設定して丸い白をオーバーレイすればいい。

**リンク**:

1. [ファッションをProcessingで描く](http://blog.livedoor.jp/reona396/archives/54669609.html)

### 86日目: 2020年12月26日（土）

**今日の進捗**:

p5.jsでテレビのノイズ（砂嵐）っぽく描画する。

- 結論から書くと、FPSが遅くてイマイチだがとりあえずそれっぽくなったので記載する。
...
- `copy()`関数は、Canvas上の任意の場所の内容をコピーして、任意の位置に貼り付ける。
- ここでは左上にて生成したノイズ画面をコピーして、他の位置に貼り付けている。

**思ったこと**:

- 全体をランダムなノイズで埋めるとFPSが相当下がったので一部をコピーしたのだが、それでもまだFPSが低い。もっと軽く動作しないものだろうか。
- p5.js使わないでCanvasを直接いじらないとだめか？

**リンク**:

1. [snow noise](https://tech.pfq.jp/lab/537/)
1. [JavaScriptでスノーノイズ(砂嵐)](https://qiita.com/akebi_mh/items/02bb9038a8b6f4944fce)
1. [copy](https://p5js.org/reference/#/p5/copy)

### 87日目: 2020年12月27日（日）

**今日の進捗**:

[ml5.js](https://ml5js.org/)とp5.jsを使ってみる。

- 今回はml5.jsのposeNetを利用する。poseNetは顔なら鼻や目などの位置を、体なら肩や膝などの位置を画像やカメラから検出できる機能。
- カメラから取り込んだ映像のうち、目の部分に黒い四角を描画するサンプルを作成する。
...
- ml5.jsを利用するには上記の1文を記述しておく。現状での最新版はv0.6.0。
...
function draw(){
  ...
  fill(0);
  rect(eyelX, eyelY, 20, 20);
  rect(eyerX, eyerY, 20, 20);
...
- poseNetで得られる体の位置はいくつかあるが、ここでは目の位置のみを利用している。
- `video.hide();`はデバッグ用のカメラ画像を隠す。

**思ったこと**:

- これを書くだけで検出できるんだから、ml5.jsってライブラリすごいな。

**リンク**:

1. [ml5js / ml5-library](https://github.com/ml5js/ml5-library)
1. [poseNet](https://ml5js.org/reference/api-PoseNet/)
1. [ml5.jsを知っているかい？](https://qiita.com/ta9ma_0k/items/fff18b072fd75fd7aaf0)

### 88日目: 2020年12月28日（月）

**今日の進捗**:

ml5.jsのbodyPixを使ってみる。

- bodyPixとは人物のシルエットを認識して、人物自身または背景をマスクできる。
...
const options = {
  outputStride: 8, // 8か16か32。デフォルトは16。人を認識する際の滑らかさ。
  segmentationThreshold: 0.6 // 0から1。デフォルトは0.5。数値が小さいほど大雑把にマスクする。
}
...
function draw(){
  ...
  if (segmentation){
    // image(segmentation.backgroundMask, 0, 0); // 背景をマスク
    image(segmentation.personMask, 0, 0); // 人をマスク
  }
...
- [オフィシャルのbodyPixのページ](https://ml5js.org/reference/api-BodyPix/)には「Returns "{raw}", "{maskBackground}", and "{maskPerson}".」と書いてあるのだが、**これは間違い**。実際は`backgroundMask`と`personMask`の2つ。最新版だとこれだけしか有効じゃないらしい。旧Verだと上記のリターン内容だったようだが、[2019年10月の変更で変わったっぽい](https://github.com/ml5js/ml5-library/commit/465d63dde622945849315b3f38851afd0c5f6100#diff-ccd09d718bdd71d3532d7c121b3f8b0ccfff6837c355687599302a7ee6ce0fd4)。
- [ここのサンプル](https://editor.p5js.org/ml5/sketches/BodyPix_Webcam)は変更後のソースなので、こっちのほうが参考になる。

**思ったこと**:

- コーディングミスかと思ってよくよく調べたら、オフィシャルのドキュメントが間違っていたでござるの巻。

**リンク**:

1. [ml5.js examples search](https://ml5js.github.io/ml5-examples/public/)
...
- 前回は「800x600のキャンバスに1ピクセルずつランダムに四角を配置したら超絶重くなった」「300x300で生成したノイズのキャンバスをコピーして残りの領域に貼り付けたけど砂嵐っぽいランダム感がない」あたりが問題だった。
- 今回は「そこそこの画面サイズでも軽快にノイズが描画できること」「完全ランダムじゃなくても砂嵐っぽく見えること」を優先した。
...
function setup(){
  ...
  const promise = new Promise((resolve, reject) => {
    let shapes = [];
    [...Array(tileNum).keys()].forEach(_ => {
      shapes.push(drawTVNoise());
    });
    resolve(shapes);
  }).then(result => {
    noises = [...result];
...
function draw(){
  ...
  for(let w=0; w<width; w += (seedSize - random(seedSize))){
    for(let h=0; h<height; h += (seedSize - random(seedSize))){
      let i = random([...Array(tileNum).keys()]);
      image(noises[i], w, h);
    }
  } 
...
function drawTVNoise(){
  ...
  let pg = createGraphics(seedSize, seedSize);
  pg.noStroke();
  
  for(let w=0; w<=seedSize;w += cellSize){
    for(let h=0; h<=seedSize;h += cellSize){
      pg.fill(random(255));
      pg.rect(w,h,cellSize, cellSize);
    }
  }
...
- `createGraphics()`は、本来とは別のCanvasを生成してオブジェクトのように貼り付けられる機能。これを使って、任意のサイズのキャンバスを作成しノイズを描画する。この際、何パターンかノイズのキャンバスを作成しておく。生成した別キャンバスを大元のキャンバスに貼り付けることで、キャンバスサイズが大きくても軽快に動作するようにした。
- 作成したノイズキャンバスを決まったパターンで貼り付けるとランダム感に欠けるので、貼り付ける位置やノイズキャンバスをランダムに変化させることで、軽快な動作とランダム感を両立させた。

**思ったこと**:

- おー、やってみるもんだ。軽さとランダムさが両立できた。やったー。

**リンク**:

1. [Create Graphics](https://p5js.jp/examples/structure-create-graphics.html)
1. [createGraphics](https://p5js.org/reference/#/p5/createGraphics)
1. [クリスマスだしp5.jsでDLA雪片を降らせてみたよ2019](https://infosmith.biz/blog/it/snowflakes-2019)
...
- p5.jsの`random()`はJSの`Math.random()`とは異なっていて、使い勝手が向上している。基本的にはp5.jsの`random()`を使えばいい。
...
function setup(){
  ...
  let hoge = [...Array(20).keys()];
  
  console.log(random(hoge));
  
  let fuga = ['foo', 'bar', 'baz'];
  
  console.log(random(fuga));
  
}

```

- 何もなく`random()`とだけ記述すれば使えるのもポイント。
...
- 引数を指定しなかった場合、0以上1未満の値を返す。
- 引数に任意の数値を指定した場合、0以上引数の値未満の値を返す。この場合、少数が返ってくるので整数で利用したい場合は`Math.ceil()`などを使う。
- 引数に配列を指定した場合、配列の要素からランダムでピックアップして返す。要素が数値でも文字列でも問題なく利用できる。

**思ったこと**:

- JSの`Math.random()`が持つ弱点を見事に解消しているので、使い勝手がすごくいいなー。
...
ReactでTypeScriptを利用する際について。

- `npx create-react-app`実行の際に`--template`オプションでTypeScriptを指定すると、TypeScript環境として構築してくれる。

```console
npx create-react-app my-app --template typescript
```

- なお、`--template typescript`とせず`--typescript`と記述しているページ（**リンク**:参照）を見つけた。どうもエイリアスになっているようで、試しに`--typescript`で実行してみたがエラーにはならなかった。ただ、オフィシャルのドキュメントに記述してある方法は`--template typescript`なのでこちらを採用することとする。
- 上記を実行すると、最終的なソースは拡張子が`.tsx`になっているはず（普段は`.jsx`）。

**思ったこと**:

- とはいえ、まだTypeScriptはイマイチ分からないけどね・・・。
- 大晦日なので今日はこれだけ。

**リンク**:

1. [create-react-app TypeScript](https://reactjs.org/docs/static-type-checking.html#typescript)
1. [Getting Started with TypeScript and React](https://create-react-app.dev/docs/adding-typescript/#getting-started-with-typescript-and-react)
1. [TypeScript を使う React プロジェクトを作成する](https://blog.ouvill.net/blog/2019-04-14--start-react-with-typescript/start-react-with-typescript/)
